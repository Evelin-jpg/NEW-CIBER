...
<div class="container-fluid display-table">
        <div class="row display-table-row">
            <div class="col-md-2 col-sm-1 hidden-xs display-table-cell v-align box" id="navigation">
                <div class="logo">
                    <a href="home.html"><img src="https://i.gifer.com/M72a.gif" alt="merkery_logo" class="hidden-xs hidden-sm">
                        <img src="https://i.gifer.com/M72a.gif" alt="merkery_logo" class="visible-xs visible-sm circle-logo">
                    </a>
                </div>
                <div class="navi">
                    <ul>
                    ...
                    echo '
                        ...
                        <li class="active"><a href="#"><i class="fa fa-user" aria-hidden="true"></i><span class="hidden-xs hidden-sm">Conexion</span></a></li>
                            ...
                            <div class="col-md-11">
                                <div class="panel panel-primary">
                                    <div class="panel-heading" style="display: flex; align-items: center; flex-wrap: wrap;">
                                        <h2 class="text-center" style="flex-basis: 100%;">Conexion</h2>
                                        <div style="width: 100%;">
                                        <?php 
                                        echo "
                                            <button class='btn btn-success' type='button' onclick='window.location.href = `conexionAdd.php?user=$user`' style='margin-bottom: 10px';>Agregar</button>";
                                        ?>
                                    ...
                                    <table class="table table-hover" id="dev-table">
                                        ...
                                        <tbody>
                                            <?php
                                                include('conecta.php');
                                                $query = "SELECT `c`.`idConexion`, `cl`.`nombre`, `e`.`equipo`, `c`.`FechaInicio`, `c`.`FechaCierre` FROM `conexion` `c` INNER JOIN `cliente` `cl` ON `c`.`idClinete` = `cl`.`idCliente` INNER JOIN `equipo` `e` ON `c`.`idEquipo` = `e`.`idEquipo` WHERE `c`.`estatus` = 1";
                                                ...
                                                while($row = mysqli_fetch_assoc($result)){
                                                    ...
                                                    <tr class='larger-row'>
                                                        ...
                                                        <td>
                                                            <form action='conexionEditar.php' method='post'>
                                                                <input type='hidden' name='editar' id='editar' value='".$row['idConexion']."'>
                                                        ...
                                                        <td>
                                                            <form action='conexionEliminar.php' method='post'>
                                                                <input type='hidden' name='borrar' value='".$row['idConexion']."'>
                                                                <input type='hidden' name='user' id='user' value='".$user."'>
